...
class CustomizedUnigramModel(UnigramModel):
    ...
    def getFeatureMap(self):
        ...
        customized_features = ['CONTAIN_NUMBERS', 'NOW=1', 'NOW=2', 'NOW=3', 'NOW=4', 'NOW>=5', 'CONTAIN_INC', 'CONTAIN_LTD', 'CONTAIN_CORP', 'CONTAIN_CO', 'CONTAIN_PLC', 
                               'CONTAIN_TRUST', 'CONTAIN_CORPORAT', 'LENGTH<10', 'LENGTH<20']
...
'''
class CustomizedUnigramModel(UnigramModel):
    def __init__(self, train_path, val_path, test_path, lam=0.1):
        super().__init__(train_path, val_path, test_path, lam)
    ...
    def getFeatureMap(self):
        ...
        self.num_of_features = len(unique_chars)
        for i, feature in enumerate(unique_chars):
    ...
    def addNGramFeatures(self, dataset):
        ...
        for i in range(m):
            ...
            for char in x:
                if dataset == "train":
                    self.F_train[self.feature_to_index[char], i] += feature_weight
                else:
                    if char in self.feature_to_index.keys():
                        F[self.feature_to_index[char], i] += feature_weight
        '''
    def addCustomizedFeatures(self, dataset):
        n = self.n
        feature_weight = self.feature_weight
...
class CustomizedBigramModel(BigramModel):
    def __init__(self, train_path, val_path, test_path, lam=0.1):
        super().__init__(train_path, val_path, test_path, lam)
    ...
    def getFeatureMap(self):
        ...
        unique_ngram_features = unique_chars + unique_bigrams
        ...
        print("Total features: " + str(len(self.num_of_features)))
